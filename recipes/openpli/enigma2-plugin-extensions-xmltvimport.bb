...
SRC_URI = "git://openpli.git.sourceforge.net/gitroot/openpli/plugin-xmltvimport;protocol=git"
...
do_install_append() {
	...
	mv ${D}/usr/lib/python2.6/site-packages/*.so ${D}/usr/lib/enigma2/python/Plugins/Extensions/${PLUGIN}
...
pkg_postinst() {
	if grep -q PLi /etc/image-version
	then
		# PLi needs no patch...
		true
	else
		[ -f /usr/bin/enigma2.sh.xmltvbak ] || {
			cp /usr/bin/enigma2.sh /usr/bin/enigma2.sh.xmltvbak
			sed '3ipython /usr/lib/enigma2/python/Plugins/Extensions/EPGImport/boot.py' /usr/bin/enigma2.sh.xmltvbak > /usr/bin/enigma2.sh
		}
	fi
}
